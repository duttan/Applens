...
interface TimesheetDao {
    ...
    @Query("select t1.ticket_Id,t1.ticketDesc,t2.TimeSheetID, t2.Logged_efforts ,t1.ticketStatus,t2.effortDate,t2.serviceID,t2.activityID from Ticket_Table t1 left join timesheet_table t2 on t1.ticket_Id =t2.ticketID and t2.effortDate= :key WHERE strftime('%Y/%m/%d',t1.ticketOpenDate) <= strftime('%Y/%m/%d',t2.effortDate) and strftime('%Y/%m/%d',t1.ticketCloseDate) >= strftime('%Y/%m/%d',t2.effortDate) and t1.ticketStatus != 'Closed'")
